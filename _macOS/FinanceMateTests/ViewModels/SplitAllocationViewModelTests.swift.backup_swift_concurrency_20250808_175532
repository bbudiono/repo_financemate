...
final class SplitAllocationViewModelTests: XCTestCase {
  ...
  override func setUp() {
    ...
    testTransaction = Transaction.create(
      in: context, amount: 200.00, category: "Test", note: "Test transaction")
    testLineItem = LineItem.create(
      in: context, itemDescription: "Test Item", amount: 100.00, transaction: testTransaction)
    try! context.save()
  }
  ...
  private func createTestSplitAllocation(
    percentage: Double, taxCategory: String, lineItem: LineItem? = nil
  ) -> SplitAllocation {
    let targetLineItem = lineItem ?? testLineItem!
    let split = SplitAllocation.create(
      in: context, percentage: percentage, taxCategory: taxCategory, lineItem: targetLineItem)
    try! context.save()
    return split
  }

  private func createMultipleSplits(
    percentages: [Double], categories: [String], lineItem: LineItem? = nil
  ) -> [SplitAllocation] {
    let targetLineItem = lineItem ?? testLineItem!
    ...
    for (index, percentage) in percentages.enumerated() {
      ...
      let split = SplitAllocation.create(
        in: context, percentage: percentage, taxCategory: category, lineItem: targetLineItem)
      splits.append(split)
    }
  ...
  func testViewModelInitialization() {
    ...
    XCTAssertEqual(
      newViewModel.splitAllocations.count, 0, "Initial split allocations array should be empty")
  ...
  func testInitializationWithPredefinedTaxCategories() {
    ...
    XCTAssertTrue(
      viewModel.predefinedCategories.contains("Business"), "Should include Business category")
    XCTAssertTrue(
      viewModel.predefinedCategories.contains("Personal"), "Should include Personal category")
    XCTAssertTrue(
      viewModel.predefinedCategories.contains("Investment"), "Should include Investment category")
    XCTAssertTrue(
      viewModel.predefinedCategories.contains("Charity"), "Should include Charity category")
    XCTAssertGreaterThan(
      viewModel.predefinedCategories.count, 3, "Should have multiple predefined categories")
  }
  ...
  func testAddSplitAllocationSuccess() async {
    ...
    XCTAssertEqual(
      viewModel.splitAllocations.first?.taxCategory, "Business", "Tax category should match")
    XCTAssertEqual(
      viewModel.totalPercentage, 60.0, accuracy: 0.01, "Total percentage should be updated")
  ...
  func testAddSplitAllocationExceedingTotalPercentage() async {
    ...
    XCTAssertNotNil(viewModel.errorMessage, "Should have validation error")
    XCTAssertTrue(
      viewModel.errorMessage?.contains("100%") == true, "Error should mention 100% limit")
    XCTAssertEqual(
  ...
  func testUpdateSplitAllocationSuccess() async {
    ...
    XCTAssertEqual(
      viewModel.splitAllocations.first?.taxCategory, "Personal", "Tax category should be updated")
    XCTAssertEqual(
      viewModel.totalPercentage, 75.0, accuracy: 0.01, "Total percentage should be updated")
    XCTAssertNil(viewModel.errorMessage, "Should have no error")
  }
  ...
  func testFetchSplitAllocationsForLineItem() async {
    ...
    XCTAssertEqual(
      viewModel.splitAllocations.count, 2, "Should have two split allocations for test line item")
    XCTAssertTrue(
      viewModel.splitAllocations.allSatisfy { $0.lineItem == testLineItem },
      "All split allocations should belong to test line item")
    XCTAssertEqual(
      viewModel.totalPercentage, 100.0, accuracy: 0.01, "Total percentage should be 100%")
  }
  ...
  func testRealTimePercentageValidation() async {
    ...
    XCTAssertEqual(
      viewModel.totalPercentage, 75.0, accuracy: 0.01, "Total percentage should be 75%")
  ...
  func testExceedingPercentageValidation() async {
    ...
    XCTAssertEqual(
      viewModel.totalPercentage, 120.0, accuracy: 0.01, "Total percentage should be 120%")
  ...
  func testZeroPercentageValidation() async {
    ...
    XCTAssertEqual(
      viewModel.remainingPercentage, 100.0, accuracy: 0.01, "Remaining percentage should be 100%")
  }
  ...
  func testAddCustomTaxCategory() {
    ...
    XCTAssertTrue(
      viewModel.availableTaxCategories.contains(customCategory), "Should include custom category")
    XCTAssertTrue(
      viewModel.customCategories.contains(customCategory), "Should be in custom categories list")
  }
  ...
  func testAddDuplicateCustomTaxCategory() {
    ...
    XCTAssertEqual(
      viewModel.customCategories.count, initialCount, "Should not add duplicate category")
  }
  ...
  func testRemoveCustomTaxCategory() {
    ...
    XCTAssertFalse(
      viewModel.customCategories.contains(customCategory), "Category should be removed")
    XCTAssertFalse(
      viewModel.availableTaxCategories.contains(customCategory),
      "Should not be in available categories")
  }
  ...
  func testCannotRemovePredefinedTaxCategory() {
    ...
    XCTAssertEqual(
      viewModel.predefinedCategories.count, initialCount,
      "Predefined categories should not be affected")
    XCTAssertTrue(
      viewModel.availableTaxCategories.contains(predefinedCategory),
      "Predefined category should remain available")
  }
  ...
  func testErrorHandlingEmptyTaxCategory() async {
    ...
    XCTAssertNotNil(viewModel.errorMessage, "Should have validation error")
    XCTAssertTrue(
      viewModel.errorMessage?.contains("category") == true, "Error should mention category")
  }
  ...
  func testQuickSplit5050() async {
    ...
    XCTAssertEqual(
      businessSplit?.percentage ?? 0, 50.0, accuracy: 0.01, "Business split should be 50%")
    XCTAssertEqual(
      personalSplit?.percentage ?? 0, 50.0, accuracy: 0.01, "Personal split should be 50%")
  ...
  func testQuickSplit7030() async {
    ...
    XCTAssertEqual(
      businessSplit?.percentage ?? 0, 70.0, accuracy: 0.01, "Business split should be 70%")
    XCTAssertEqual(
      personalSplit?.percentage ?? 0, 30.0, accuracy: 0.01, "Personal split should be 30%")
    XCTAssertTrue(viewModel.isValidSplit, "Split should be valid")
  }
  ...
  func testSplitAllocationLineItemRelationship() {
    ...
    XCTAssertEqual(
      splitAllocation.lineItem, testLineItem, "Split allocation should be linked to line item")
  ...
  func testFetchSplitAllocationsPerformance() async {
    ...
    XCTAssertEqual(viewModel.splitAllocations.count, 200, "Should fetch all split allocations")
  ...
  func testRealTimeValidationPerformance() async {
    ...
    let startTime = CFAbsoluteTimeGetCurrent()
    for _ in 0..<1000 {
      _ = viewModel.isValidSplit
      _ = viewModel.totalPercentage
      _ = viewModel.remainingPercentage
    }
    let timeElapsed = CFAbsoluteTimeGetCurrent() - startTime
