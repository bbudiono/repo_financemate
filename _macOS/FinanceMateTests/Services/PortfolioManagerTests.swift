...
final class PortfolioManagerTests: XCTestCase {
  ...
  func testRecordBuyTransaction() throws {
    ...
    XCTAssertEqual(
      ...
      "Should calculate average cost basis including fees")
    XCTAssertEqual(
      updatedHolding.totalCostBasis, 10569.95, accuracy: 0.01, "Should calculate total cost basis")
  ...
  func testCalculatePortfolioValue() throws {
    ...
    XCTAssertEqual(
      portfolioValue.totalMarketValue, expectedValue, "Should calculate total market value")
  ...
  func testCGTCalculationForSale() throws {
    ...
    XCTAssertEqual(
      sellTransaction.discountedCapitalGain!, discountedGain, accuracy: 0.01,
  ...
  func testFrankingCreditCalculation() throws {
    ...
    XCTAssertEqual(
      dividend.frankingCredit, expectedFrankingCredit, accuracy: 0.01,
      "Should calculate franking credit")
    XCTAssertEqual(
      dividend.grossDividend, expectedGrossDividend + expectedFrankingCredit, accuracy: 0.01,
      "Should calculate gross dividend")
  }
  ...
  func testPortfolioIsolationBetweenEntities() throws {
    ...
    XCTAssertEqual(
      entity1Portfolios.first?.id, portfolio1.id, "Should return correct portfolio for entity 1")
    XCTAssertEqual(
      entity2Portfolios.first?.id, portfolio2.id, "Should return correct portfolio for entity 2")
  }
  ...
  func testLargePortfolioPerformance() throws {
    ...
    XCTAssertLessThan(
      processingTime, 2.0, "Large portfolio calculation should complete within 2 seconds")
    XCTAssertGreaterThan(
      portfolioValue.totalMarketValue, 0, "Should calculate non-zero portfolio value")
  }
  ...
  func testPortfolioCaching() throws {
    ...
    XCTAssertNotNil(secondFetch, "Second fetch should return portfolio")
  ...
  func testHoldingCaching() throws {
    ...
    XCTAssertNotNil(secondFetch, "Second fetch should return holding")
  ...
  func testPortfolioValueCaching() throws {
    ...
    XCTAssertEqual(
      firstCalculation.totalMarketValue, secondCalculation.totalMarketValue,
      "Cached calculation should return same value")
    XCTAssertEqual(
      firstCalculation.totalCostBasis, secondCalculation.totalCostBasis,
      "Cached calculation should return same value")
  }
  ...
  func testCacheInvalidationAfterTransaction() throws {
    ...
    XCTAssertGreaterThan(
      newValue.totalCostBasis, initialValue.totalCostBasis,
      "New cost basis should be higher after additional purchase")
  }
  ...
  func testBatchTransactionProcessing() throws {
    ...
    XCTAssertEqual(
      updatedHolding2.quantity, 50.0, accuracy: 0.01, "Holding 2 should have 50 shares")

    // Verify cost basis calculations
    let expectedCostBasis1 = (100.0 * 100.0 + 19.95) + (25.0 * 102.0 + 19.95)
    XCTAssertEqual(
      updatedHolding1.totalCostBasis, expectedCostBasis1, accuracy: 0.01,
      "Holding 1 cost basis should be correct")

    let expectedCostBasis2 = (50.0 * 150.0 + 19.95)
    XCTAssertEqual(
      updatedHolding2.totalCostBasis, expectedCostBasis2, accuracy: 0.01,
  ...
  func testBatchTransactionWithMixedTypes() throws {
    ...
    // Verify realized gain/loss is calculated for sell transaction
    let sellTransaction = processedTransactions.first { $0.type == "sell" }
    XCTAssertNotNil(
      sellTransaction?.realizedGainLoss, "Should calculate realized gain/loss for sell")
  ...
  func testBatchTransactionPerformance() throws {
    ...
    for i in 0..<100 {
      let transaction = InvestmentTransactionData(
        holdingId: holding.id!,
        type: i % 4 == 0 ? .sell : .buy,  // Every 4th transaction is a sell
        quantity: Double(10 + i),
        price: Double(100 + i),
        transactionFee: 19.95,
        transactionDate: Date()
      )
    ...
    // Then: Should complete within performance target
    XCTAssertEqual(processedTransactions.count, 100, "Should process all 100 transactions")
