...
public class MultiLLMAgentCoordinator: ObservableObject {
    ...
    @Published public var performance = CoordinatorPerformance()
    ...
    public init(
        ...
        self.frontierSupervisor = FrontierModelSupervisor(defaultModel: .claudeSonnet4)
    ...
    public func hasAgentFor(role: AgentRole) -> Bool {
        registeredAgents.contains { $0.role == role && $0.status == .available }
    }

    public func getAgentsFor(role: AgentRole) -> [MultiLLMAgent] {
        registeredAgents.filter { $0.role == role && $0.status == .available }
    }
    ...
    public func getAgentLoadDistribution() -> [String: Int] {
        loadBalancer.getCurrentLoadDistribution()
    }
    ...
    private func decomposeTask(_ task: MultiLLMTask) async -> [SubTask] {
        ...
        await frontierSupervisor.decomposeTask(task)
    }

    private func assignTasksToAgents(_ subtasks: [SubTask]) -> [TaskAssignment] {
        loadBalancer.assignTasksToAgents(subtasks, agents: registeredAgents)
    }
    ...
    private func executeAssignedTasks(_ assignments: [TaskAssignment], supervision: SupervisionLevel) async throws -> [TaskResult] {
        ...
        for assignment in assignments {
            ...
            if supervision == .full {
                _ = await frontierSupervisor.reviewTaskResult(result)
    ...
    private func validateGraphStructure(_ graph: MultiLLMGraph) -> GraphValidation {
        ...
        GraphValidation(isValid: true, error: nil)
    }
...
// FrontierModel enum is defined in FrontierModelsService.swift
