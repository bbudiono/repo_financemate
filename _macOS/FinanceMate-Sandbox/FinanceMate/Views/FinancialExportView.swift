...
struct FinancialExportView: View {
    ...
    @State private var includeCategories: Bool = true
    @State private var includeTaxInfo: Bool = false
    ...
    private var exportOptionsSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            VStack(spacing: 8) {
                Toggle("Include Categories", isOn: $includeCategories)
                    .toggleStyle(.checkbox)

                Toggle("Include Tax Information", isOn: $includeTaxInfo)
                    .toggleStyle(.checkbox)

                if selectedFormat == .googleSheets {
                    Toggle("Open in browser after export", isOn: .constant(true))
                        .toggleStyle(.checkbox)
                        .disabled(true)
                        .opacity(0.6)
                }
    ...
    private var exportActionSection: some View {
        VStack(spacing: 16) {
            ...
            Button(action: performExport) {
                HStack {
                    Image(systemName: isExporting ? "arrow.down.circle" : formatIcon)
                    Text(isExporting ? "Exporting..." : "Export \(selectedFormat.rawValue)")
                }
                .frame(maxWidth: .infinity)
                .padding()
                .background(isExporting || filteredDataCount == 0 ? Color.gray : formatColor)
                .foregroundColor(.white)
                .cornerRadius(10)
            }
    ...
    private var formatIcon: String {
        ...
        case .googleSheets: return "globe"
        }
    }
    ...
    private var formatColor: Color {
        ...
        case .googleSheets: return .purple
        }
    }
    ...
    private var formatDescription: String {
        ...
        case .googleSheets: return "Google Sheets Online"
        }
    }

    private var formatDetails: String {
        switch selectedFormat {
        case .csv: return "Compatible with most spreadsheet applications and financial software"
        case .excel: return "Advanced Excel format with enhanced formatting and category support"
        case .json: return "Structured data format suitable for developers and API integration"
        case .googleSheets: return "Creates a new Google Sheets document accessible from any device"
        }
    }
