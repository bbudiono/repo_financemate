...
class SSOAuthenticationTests: XCTestCase {
    ...
    func testTokenSaveAndRetrieve() {
        ...
        XCTAssertNotNil(tokenManager.getToken(for: .apple), "Should have valid Apple token")
    }
    ...
    func testTokenClearAll() {
        ...
        XCTAssertNotNil(tokenManager.getToken(for: .apple), "Should have Apple token")
        XCTAssertNotNil(tokenManager.getToken(for: .google), "Should have Google token")
        
        // Clear all tokens
        tokenManager.clearAllTokens()
        
        XCTAssertNil(tokenManager.getToken(for: .apple), "Should not have Apple token after clear")
        XCTAssertNil(tokenManager.getToken(for: .google), "Should not have Google token after clear")
    }
    ...
    func testTokenManagerSecurity() {
        ...
        XCTAssertNil(tokenManager.getToken(for: .apple), "Apple token should be cleared")
        XCTAssertEqual(tokenManager.getToken(for: .google), googleToken, "Google token should remain")
    }
