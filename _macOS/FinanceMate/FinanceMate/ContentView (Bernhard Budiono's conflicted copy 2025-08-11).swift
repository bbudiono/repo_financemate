...
private struct UnifiedEmailReceiptView: View {
    ...
    @StateObject private var gmailProcessor = SimplifiedGmailProcessor()
    @State private var showingConfigurationHelp = false
    @State private var showingExpenseDetails = false
    @State private var selectedExpense: SimplifiedGmailExpense?
    ...
    private var leftPanelView: some View {
        ScrollView {
            VStack(spacing: 20) {
                ...
                // Processing Status
                if gmailProcessor.isProcessing {
                    processingStatusSection
                }
                
                // Error Display
                if let error = gmailProcessor.processingError {
    ...
    private var configurationStatusSection: some View {
        VStack(spacing: 8) {
            HStack {
                Image(systemName: gmailProcessor.isConfigured ? "checkmark.circle.fill" : "exclamationmark.triangle.fill")
                    .foregroundColor(gmailProcessor.isConfigured ? .green : .orange)
                    .font(.title3)
                
                VStack(alignment: .leading, spacing: 2) {
                    Text(gmailProcessor.isConfigured ? "Production Ready" : "Configuration Required")
                        .font(.subheadline)
                        .fontWeight(.medium)
                        .foregroundColor(gmailProcessor.isConfigured ? .green : .orange)
                    
                    Text(gmailProcessor.configurationStatus)
                        .font(.caption)
                        .foregroundColor(.secondary)
                        .lineLimit(3)
                }
    ...
    private var processingControlsSection: some View {
        VStack(spacing: 12) {
            if gmailProcessor.isConfigured {
                ...
                .disabled(gmailProcessor.isProcessing)
            } else {
                Button("Setup OAuth") {
                    Task {
    ...
    private var processingStatusSection: some View {
        VStack(spacing: 8) {
            ...
            Text(gmailProcessor.processingStatus)
                .font(.caption)
                .foregroundColor(.secondary)
                .multilineTextAlignment(.center)
        }
        ...
        VStack(spacing: 12) {
            HStack {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("\(gmailProcessor.expenses.count) expense\(gmailProcessor.expenses.count == 1 ? "" : "s") extracted")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                }
                ...
                VStack(alignment: .trailing) {
                    Text("AUD $\(String(format: "%.2f", gmailProcessor.totalExpenses))")
                        .font(.title3)
                        .fontWeight(.bold)
                        .foregroundColor(.blue)
            ...
            // Quick stats row
            if !gmailProcessor.expenses.isEmpty {
                HStack(spacing: 20) {
                    statItem("Receipts", "\(gmailProcessor.expenses.count)", .blue)
                    statItem("Categories", "\(Set(gmailProcessor.expenses.map { $0.category }).count)", .green)
                    
                    let avgConfidence = gmailProcessor.expenses.reduce(0) { $0 + $1.confidenceScore } / Double(max(gmailProcessor.expenses.count, 1))
    ...
    private var expenseTableContent: some View {
        ScrollView {
            LazyVStack(spacing: 12) {
                if gmailProcessor.expenses.isEmpty && !gmailProcessor.isProcessing {
                    emptyExpenseTableState
                } else {
                    ForEach(gmailProcessor.expenses, id: \.id) { expense in
    ...
    private var emptyExpenseTableState: some View {
        VStack(spacing: 20) {
            ...
            VStack(spacing: 8) {
                ...
                Text(gmailProcessor.isConfigured ? 
    ...
    private func realGmailExpenseRow(_ expense: SimplifiedGmailExpense) -> some View {
        Button(action: {
            ...
            HStack(spacing: 16) {
                ...
                VStack {
                    Image(systemName: iconForCategory(expense.category))
                        ...
                        .background(colorForCategory(expense.category))
                        .cornerRadius(12)
                }
    ...
    private func authenticateRealGmail() async {
        await gmailProcessor.authenticateGmail()
    }
    
    private func processRealGmailReceipts() async {
        await gmailProcessor.processGmailReceipts()
    }
}
        ...
        NavigationView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    detailRow("Email ID", expense.emailId)
                }
