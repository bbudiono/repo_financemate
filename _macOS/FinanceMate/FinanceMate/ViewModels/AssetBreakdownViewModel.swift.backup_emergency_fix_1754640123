...
class AssetBreakdownViewModel: ObservableObject {
    ...
    func fetchAssetBreakdown() {
        ...
        performAsyncDataFetch()
    }

    /// Perform asynchronous asset breakdown data fetch
    private func performAsyncDataFetch() {
        isLoading = true
        errorMessage = nil

        do {
            let breakdown = assetBreakdownService.getAssetBreakdown(for: entity)
    ...
    func sortAssets(by order: AssetSortOrder) {
        sortOrder = order
        // EMERGENCY FIX: Removed Task block - immediate execution
        let sortedAssets = assetBreakdownService.getSortedAssets(by: order, entity: entity)
            MainActor.run {
    ...
    func addAsset(
        ...
        // EMERGENCY FIX: Removed Task block - immediate execution
        let result = assetBreakdownService.addAsset(
                name: name,
                type: type,
    ...
    func updateAsset(_ asset: Asset, newValue: Double) {
        // EMERGENCY FIX: Removed Task block - immediate execution
        let success = assetBreakdownService.updateAsset(asset, newValue: newValue)
    ...
    func deleteAsset(_ asset: Asset) {
        // EMERGENCY FIX: Removed Task block - immediate execution
        let success = assetBreakdownService.deleteAsset(asset)
