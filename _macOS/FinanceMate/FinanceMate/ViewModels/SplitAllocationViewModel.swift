...
final class SplitAllocationViewModel: ObservableObject {
    ...
    @Published var newSplitPercentage: Double = 0.0
    @Published var selectedTaxCategory: String = ""
    ...
    func addSplitAllocation(to lineItem: LineItem) async {
        ...
        guard validateTotalPercentage(adding: newSplitPercentage) else {
            errorMessage = "Adding this percentage would exceed 100%. Remaining available: \(String(format: "%.2f", remainingPercentage))%"
            return
        }
