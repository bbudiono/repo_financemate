...
public class TokenStorage {
    ...
    func storeTokens(_ tokenData: TokenData, for provider: OAuth2Provider) throws {
        let account = "\(provider.rawValue)_tokens"
        
        do {
            let data = try JSONEncoder().encode(tokenData)
            try storeInKeychain(data: data, account: account)
        } catch {
            throw TokenStorageError.storageError("Failed to store tokens for \(provider.displayName): \(error.localizedDescription)")
        }
    }
    
    func retrieveTokens(for provider: OAuth2Provider) throws -> TokenData? {
        let account = "\(provider.rawValue)_tokens"
        
        do {
            guard let data = try retrieveFromKeychain(account: account) else {
                return nil
            }
    ...
    func deleteTokens(for provider: OAuth2Provider) throws {
        let account = "\(provider.rawValue)_tokens"
    ...
    func hasValidTokens(for provider: OAuth2Provider) -> Bool {
        do {
            guard let tokenData = try retrieveTokens(for: provider) else {
                return false
            }
    ...
    private func retrieveFromKeychain(account: String) throws -> Data? {
        var query: [String: Any] = [
            kSecClass as String: kSecClassGenericPassword,
            kSecAttrService as String: serviceName,
            kSecAttrAccount as String: account,
    ...
    func refreshTokenIfNeeded(for provider: OAuth2Provider, refreshHandler: (String) throws -> TokenData) throws -> TokenData? {
        guard let currentTokens = try retrieveTokens(for: provider) else {
            return nil
        }
        ...
        if timeUntilExpiry <= refreshThreshold {
            ...
            do {
                let newTokenData = try refreshHandler(refreshToken)
                try storeTokens(newTokenData, for: provider)
                return newTokenData
            } catch {
    ...
    func validateTokenSecurity(for provider: OAuth2Provider) throws -> TokenSecurityInfo {
        guard let tokenData = try retrieveTokens(for: provider) else {
