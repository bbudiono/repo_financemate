...
public class OptimizedNetWealthService: ObservableObject {
    ...
    public func calculateCurrentNetWealthOptimized() -> NetWealthResult {
        ...
        let result = baseService.calculateCurrentNetWealth()
    ...
    public func getWealthTrendOptimized() -> [WealthTrendDataPoint] {
        let cacheKey = WealthCacheKey(
            entityId: entityId,
            calculationType: .netWealth,
            dateRange: dateRange,
        ...
        let result = processBatchWealthTrend(dateRange: dateRange, entityId: entityId)
    ...
    public func calculateMultiEntityWealthOptimized() -> MultiEntityWealthBreakdown? {
        let cacheKey = WealthCacheKey(
        ...
        let entities = fetchEntitiesInBatches()
        
        guard !entities.isEmpty else { return nil }
        
        // Process entities in parallel
        let entityBreakdowns = processEntitiesInParallel(entities)
        
        // Calculate consolidated metrics
        let consolidatedWealth = calculateConsolidatedWealth(from: entityBreakdowns)
        let crossEntityAnalysis = calculateCrossEntityAnalysisOptimized(
            entityBreakdowns: entityBreakdowns,
            consolidatedWealth: consolidatedWealth
        )
        let performanceMetrics = calculatePerformanceMetricsOptimized(entityBreakdowns: entityBreakdowns)
    ...
    public func getAssetBreakdownOptimized() -> AssetBreakdownResult {
        let cacheKey = WealthCacheKey(
        ...
        let result = calculateAssetBreakdownWithPreprocessing(entity: entity)
    ...
    private func processBatchWealthTrend() -> [WealthTrendDataPoint] {
        return withTaskGroup(of: [WealthTrendDataPoint].self) { group in
...
// COMPREHENSIVE FIX: Removed ALL Swift Concurrency patterns to eliminate TaskLocal crashes
        [weak self] in
                    self?.baseService.getWealthTrend(for: chunk) ?? []
            }
            
            var allDataPoints: [WealthTrendDataPoint] = []
            for chunkResults in group {
    ...
    private func processEntitiesInParallel() -> [EntityWealthBreakdown] {
        return withTaskGroup(of: EntityWealthBreakdown?.self) { group in
...
// COMPREHENSIVE FIX: Removed ALL Swift Concurrency patterns to eliminate TaskLocal crashes
        [weak self] in
                        self?.calculateEntityWealthOptimized(entity: entity)
                }
            }
    ...
    private func fetchEntitiesInBatches() -> [FinancialEntity] {
    ...
    private func calculateEntityWealthOptimized() -> EntityWealthBreakdown? {
        ...
        do {
            ...
            let (assetMetrics, liabilityMetrics, risk) = (assetCalculations, liabilityCalculations, riskMetrics)
    ...
    private func calculateAssetBreakdownWithPreprocessing() -> AssetBreakdownResult {
        ...
        do {
            ...
            let breakdown = processAssetsInParallel(assets)
    ...
    private func calculateAssetMetricsParallel() -> (totalValue: Double, allocation: [AssetAllocationData]) {
    ...
    private func calculateLiabilityMetricsParallel() -> (totalValue: Double, breakdown: [LiabilityBreakdownData]) {
    ...
    private func calculateRiskMetricsParallel() -> EntityRiskMetrics {
        ...
        let (concentration, liquidity, leverage) = (concentrationRisk, liquidityRisk, leverageRatio)
    ...
    private func logPerformanceMetric(operation: String, duration: TimeInterval, cached: Bool) {
        ...
        print("🚀 Performance: \(operation) completed in \(String(format: "%.3f", duration))s [\(cacheStatus)]")
    }
    ...
    private func processAssetsInParallel() -> (byCategory: [String: [Asset]], totalValue: Double, categoryTotals: [String: Double]) {
    ...
    private func calculateLiquidityRisk() -> Double {
        guard totalAssets > 0 else { return 0 }
    ...
    private func calculateLeverageRatio() -> Double {
        guard totalAssets > 0 else { return 0 }
    ...
    private func calculateCrossEntityAnalysisOptimized() -> CrossEntityAnalysis {
    ...
    private func calculatePerformanceMetricsOptimized() -> EntityPerformanceMetrics {
