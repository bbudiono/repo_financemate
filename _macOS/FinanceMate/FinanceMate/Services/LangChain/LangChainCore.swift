...
public class LangChainPromptManager: ObservableObject {
    ...
    public func createPrompt(template: String, variables: [String: Any]) async throws -> LangChainPrompt {
        let prompt = LangChainPrompt(
            id: UUID().uuidString,
            template: template,
            variables: variables,
            renderedText: try renderTemplate(template: template, variables: variables)
        )
        return prompt
    }
    ...
    private func renderTemplate(template: String, variables: [String: Any]) throws -> String {
...
public class LangChainMemoryManager: ObservableObject {
    ...
    public func save(key: String, value: Any) async throws {
    ...
    public func load(key: String) async throws -> Any? {
    ...
    public func getMemoryUsage() -> Double {
        return Double(memory.count) * 0.1 // Simplified calculation
    }
}
    ...
    public func render(with variables: [String: Any]) throws -> String {
        ...
        for variable in self.variables {
            ...
            if let value = variables[variable] {
                ...
                throw LangChainError.memoryError("Missing variable: \(variable)")
            }
        }
...
// MARK: - LangChain Prompt

public struct LangChainPrompt: Identifiable {
    public let id: String
    public let template: String
    public let variables: [String: Any]
    public let renderedText: String
    
    public init(id: String, template: String, variables: [String: Any], renderedText: String) {
        self.id = id
        self.template = template
        self.variables = variables
