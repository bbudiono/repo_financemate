...
public class LangGraphFramework: ObservableObject {
    ...
    @Published public var systemMetrics: LangGraphMetrics = LangGraphMetrics()
    ...
    // MARK: - Integration with MLACS and LangChain
    ...
    public init(configuration: LangGraphConfiguration = LangGraphConfiguration.default, mlacsFramework: MLACSFramework? = nil, langChainFramework: LangChainFramework? = nil) {
    ...
    public func getGraph(id: String) -> LangGraph? {
        return graphRegistry[id]
    }
    
    public func getAllGraphs() -> [LangGraph] {
        return Array(graphRegistry.values)
    }
    ...
    public func loadGraphState(graphId: String) async throws -> [String: Any]? {
        return try await stateManager.loadState(graphId: graphId)
    }
