...
public class MemoryMonitor: ObservableObject {
    ...
    @Published public var currentMemoryUsage = MemoryUsage(
        physical: 0,
        virtual: 0,
        resident: 0,
        shared: 0,
    ...
    public func getCurrentMemoryUsage() -> MemoryUsage {
        return SystemInfo.getSystemMemoryUsage()
    }
    ...
    private func severityForEventType(_ type: MemoryEventType) -> CrashSeverity {
        switch type {
        case .critical:
            return .critical
