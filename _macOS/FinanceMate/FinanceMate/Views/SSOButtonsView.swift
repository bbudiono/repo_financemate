...
public struct SSOButtonsView: View {
    ...
    public var body: some View {
        ...
        .onAppear {
            print("🔍 UNIFORM SSO SECTION: onAppear called - Uniform SSO buttons should be visible")
        }
    }
    ...
    private var appleSignInButton: some View {
        SignInWithAppleButton(
            onRequest: { request in
                ...
                print("🍎 UNIFORM SSO: Apple Sign-In onRequest called - setting up authorization request")
            },
            onCompletion: { result in
                print("🍎 UNIFORM SSO: Apple Sign-In onCompletion called with result: \(result)")
                switch result {
                case .success(let authorization):
                    print("🍎 UNIFORM SSO: SUCCESS - Processing authorization")
                    // EMERGENCY FIX: Removed Task block - immediate execution
        authViewModel.processAppleSignInCompletion(authorization)
                case .failure(let error):
                    print("🍎 UNIFORM SSO: FAILURE - \(error)")
    ...
    private var microsoftSignInButton: some View {
        Button(action: {
            ...
            // EMERGENCY FIX: Removed Task block - immediate execution
        authViewModel.authenticateWithOAuth2(provider: .microsoft)
        }) {
            HStack(spacing: 12) {
                if authViewModel.isLoading {
                    ProgressView()
