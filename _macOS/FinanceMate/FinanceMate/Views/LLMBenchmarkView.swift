...
struct LLMBenchmarkView: View {
    ...
    private func getSampleTestHistory() -> [BenchmarkSession] {
        return [
            ...
            BenchmarkSession(id: UUID(), suite: .general, timestamp: Date().addingTimeInterval(-86400), modelCount: 2),
            BenchmarkSession(id: UUID(), suite: .financial, timestamp: Date().addingTimeInterval(-172800), modelCount: 4)
        ]
    }
