...
struct LineItemEntryView: View {
  ...
  @State private var selectedLineItemForSplit: LineItem?
      ...
      Text(value)
        ...
        .foregroundColor(
          isHighlighted
            ? .orange
            : isRemaining
              ? (transaction.amount - viewModel.calculateTotalAmount() < 0 ? .red : .green)
              : .primary
        )
    }
    .padding(.vertical, 4)
  ...
  private var actionButtonsSection: some View {
    VStack(spacing: 12) {
      Button(action: {
        // If there's only one line item, use it directly
        if viewModel.lineItems.count == 1 {
          selectedLineItemForSplit = viewModel.lineItems.first
        } else {
          // For multiple line items, show a selection menu
          // For now, use the first item as default
          selectedLineItemForSplit = viewModel.lineItems.first
        }
  ...
  private var isFormValid: Bool {
    return !itemDescription.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
      && itemDescription.count <= 200 && !amount.isEmpty && Double(amount) != nil
      && (Double(amount) ?? 0) > 0
  }
  ...
  private func addLineItem() async {
    ...
    viewModel.newLineItem.itemDescription = itemDescription.trimmingCharacters(
