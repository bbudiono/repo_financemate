...
def test_settings_multi_section_components():
    """Settings must have modular components extracted (I-Q-I Protocol KISS fix)"""
    settings_view = MACOS_ROOT / "FinanceMate/SettingsView.swift"
    if not settings_view.exists():
        log_test("test_settings_multi_section_components", "FAIL", "SettingsView missing")
        return False

    settings_lines = len(open(settings_view).readlines())

    # Check 7 extracted Settings components exist
    settings_components = [
        "Views/Settings/SettingsSidebar.swift",
        "Views/Settings/SettingsContent.swift",
        "Views/Settings/ProfileSection.swift",
        "Views/Settings/SecuritySection.swift",
        "Views/Settings/APIKeysSection.swift",
        "Views/Settings/ConnectionsSection.swift",
        "Views/Settings/AutomationSection.swift"
    ]

    missing = [c for c in settings_components if not (MACOS_ROOT / "FinanceMate" / c).exists()]

    success = settings_lines < 100 and not missing
    log_test("test_settings_multi_section_components", "PASS" if success else "FAIL",
             f"SettingsView: {settings_lines}L, Components: {7-len(missing)}/7")
    assert success, f"Settings incomplete - {settings_lines}L, Missing: {missing}"
    return True

def test_gmail_filter_components():
    """Gmail filters extracted to modular components (I-Q-I Protocol KISS fix)"""
    filter_bar = MACOS_ROOT / "FinanceMate/Views/Gmail/GmailFilterBar.swift"
    if not filter_bar.exists():
        log_test("test_gmail_filter_components", "FAIL", "GmailFilterBar missing")
        return False

    filter_lines = len(open(filter_bar).readlines())

    # Check 6 filter components exist
    filter_components = [
        "Views/Gmail/Filters/DateFilterMenu.swift",
        "Views/Gmail/Filters/MerchantFilterMenu.swift",
        "Views/Gmail/Filters/CategoryFilterMenu.swift",
        "Views/Gmail/Filters/AmountFilterMenu.swift",
        "Views/Gmail/Filters/ConfidenceFilterMenu.swift",
        "Views/Gmail/Filters/FilterPill.swift"
    ]

    missing = [c for c in filter_components if not (MACOS_ROOT / "FinanceMate" / c).exists()]

    success = filter_lines < 100 and not missing
    log_test("test_gmail_filter_components", "PASS" if success else "FAIL",
             f"GmailFilterBar: {filter_lines}L, Components: {6-len(missing)}/6")
    assert success, f"Gmail filters incomplete - {filter_lines}L, Missing: {missing}"
    return True

def test_design_system_components():
    """Design system components exist (Toast, Spacing, Glassmorphism)"""
    components = {
        "ToastNotification.swift": ["ToastNotification", "isSuccess", "message"],
        "SpacingSystem.swift": ["SpacingSystem", "tiny", "small", "medium", "large"],
        "GlassmorphismBackground.swift": ["GlassmorphismBackground", "VisualEffectBlur"]
    }

    missing_files = []
    missing_features = {}

    for filename, required_features in components.items():
        filepath = MACOS_ROOT / "FinanceMate" / filename
        if not filepath.exists():
            ...
            if missing:
                missing_features[filename] = missing

    success = not missing_files and not missing_features
    log_test("test_design_system_components", "PASS" if success else "FAIL",
             f"Files: 3/{3-len(missing_files)}, Features: Complete" if success else f"Missing: {missing_files or missing_features}")
    assert success, f"Design system incomplete - Files: {missing_files}, Features: {missing_features}"
    return True
...
def get_test_groups():
    ...
    return [
        ...
        ("COMPONENT ARCHITECTURE", [test_settings_multi_section_components, test_gmail_filter_components, test_design_system_components]),
