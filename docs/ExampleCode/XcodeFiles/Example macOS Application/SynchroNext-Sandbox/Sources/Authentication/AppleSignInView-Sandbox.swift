// Purpose: Apple Sign In view with native Apple authentication button (Sandbox).
// Issues & Complexity: Core UI component with Apple authentication (Sandbox).
// Ranking/Rating: 95% (Code), 92% (Problem) - Essential UI component for Apple SSO (Sandbox).

import SwiftUI
import AuthenticationServices

/// View for Sign in with Apple functionality using native Apple authentication button (Sandbox).
public struct AppleSignInViewSandbox: View {
    ...
    // For Apple Sign In
    private var appleAuthProvider = AppleAuthProviderSandbox()
    
    // For Google Sign In
    private var googleAuthProvider = GoogleAuthProviderSandbox()
    ...
    public var body: some View {
        VStack(spacing: 20) {
            ...
            SignInWithAppleButton(
                ...
                onCompletion: { result in
                    ...
                    case .failure(let error):
                        ...
                        if self.appleAuthProvider.onSignInFailure == nil { // Check if the external handler wants to display its own
                            self.alertTitle = "Apple Sign-In Error (Sandbox)"
            ...
            Button(action: {
                guard let window = NSApplication.shared.windows.first(where: { $0.isKeyWindow }) ?? NSApplication.shared.windows.first else {
                    self.alertTitle = "Google Sign-In Error (Sandbox)"
        ...
        .onAppear {
            appleAuthProvider.onSignInSuccess = { userIdentifier, fullName, email in
                self.alertTitle = "Apple Sign-In Success (Sandbox)"
            ...
            appleAuthProvider.onSignInFailure = { errorDetails in
                self.alertTitle = "Apple Sign-In Failed (Sandbox)"
                self.alertMessage = errorDetails
                self.showAlert = true
            }

            googleAuthProvider.onSignInSuccess = { idToken, fullName, email in
                self.alertTitle = "Google Sign-In Success (Sandbox)"
                var successMessage = "Google User Authenticated! (Sandbox)"
                if let name = fullName, !name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                    successMessage += "\\nName: \\(name)"
                }
            ...
            googleAuthProvider.onSignInFailure = { error in
                self.alertTitle = "Google Sign-In Failed (Sandbox)"
                self.alertMessage = error.localizedDescription
                self.showAlert = true
            }
        }
    ...
    static var previews: some View {
        AppleSignInViewSandbox(
            onSignIn: { credential in
                print("Preview (Sandbox): Apple Signed in. User ID: \\(credential.user)")
            },
            onError: { error in
                print("Preview (Sandbox): Apple Sign-In Error: \\(error.localizedDescription)")
            }
        )
    }
} 