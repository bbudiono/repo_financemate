// Purpose: Shared authentication types and structures for SSO providers.
// Issues & Complexity: Implements common authentication state and error handling.
// Ranking/Rating: 95% (Code), 90% (Problem) - Shared authentication types across providers.

import Foundation

/// Represents a user with authentication details
public struct AuthUserSandbox: Identifiable, Equatable {
...
public enum AuthStateSandbox: Equatable {
    case unknown
    case authenticating
    case authenticated
    case signedOut
    case error(AuthErrorSandbox)
    
    public static func == (lhs: AuthStateSandbox, rhs: AuthStateSandbox) -> Bool {
        switch (lhs, rhs) {
...
public enum AuthErrorSandbox: Error, Equatable {
    case userCancelled
...
public struct TokenKeysSandbox {
    // Apple token keys
...
public struct AppleUserInfoSandbox: Codable, Equatable {
...
public struct GoogleUserInfoSandbox: Codable, Equatable {
    public let userId: String
    public let email: String?
