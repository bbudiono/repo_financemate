    ...
    public var body: some View {
        VStack(spacing: 20) {
            ...
            Button(action: {
                ...
                googleAuthProvider.signIn(presentingWindow: window) { result in
                    switch result {
                    case .success(let userSession):
                        self.alertTitle = "Google Sign-In Success"
                        var successMessage = "Google User Authenticated!"
                        if let name = userSession.fullName, !name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                            successMessage += "\nName: \(name)"
                        }
                        if let mail = userSession.email, !mail.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                            successMessage += "\nEmail: \(mail)"
                        }
