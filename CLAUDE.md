...
* **MANDATORY: CONSISTENT, NATURAL CHAT EXPERIENCES THAT ARE CONTEXT AWARE AND SMART.** Ensure that the chat works in a multi-turn fashion, able to consistently answer queries back to back maintaining a consistent chat experience over as many turns as possible that the context window allows. This means you need to ensure full chat context is maintained and you need to ensure that the implementation covers the ability to handle user queries requesting information from the information within the application and/or external sources such as the MCP Database, etc.
