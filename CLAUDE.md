...
- **MANDATORY:** Utilise existing OAuth Flows for both SSO on login (to complement the Apple SSO) and also the Gmail transaction/extraction feature. That way a user can simply log in once, to both the application and the email/transaction killer feature.
- **MANDATORY:** For E2E testing to be considered "passing" - ensure an objective analysis of what the "end-state" should be; for example if SSO is the feature being tested, then ensure login works both for back and front end UI's. If for example, it is the gmail extraction, then the end-state *SHOULD* be emails being retrieved and visible. i.e. App Window screenshot capturing the intended feature.
- **MANDATORY:** Ensure you have checked the code for front-end features and components first before running an E2E test for new features built
- **MANDATORY:** The agent is strictly prohibited from using AppleScript for any testing, automation, or data capture tasks. All UI interactions and verifications MUST be performed using native, framework-based automation.
- **MANDATORY:** All UI automation and user-interaction simulation MUST be executed exclusively through the XCTest framework (specifically XCUITest). This ensures that interactions are scoped to the test runner and do not interfere with the live user's system cursor or keyboard focus.
- **MANDATORY:** Visual snapshots for AI verification MUST be captured programmatically using the non-intrusive XCUIScreenshot or XCUIElement.screenshot() APIs within an XCTest. The use of system-wide tools like the screencapture command-line utility during a test run is forbidden.
- **MANDATORY:** All test execution, including the launching of the application under test and the capturing of visual snapshots, MUST occur within a sandboxed and isolated environment (e.g., an Xcode Simulator or a dedicated, non-interactive CI session) to prevent test windows or artifacts from ever appearing on the user's active display.
- **MANDATORY:**  Verification of captured snapshots using a multi-modal LLM MUST be performed programmatically via silent, background API calls. The AI's analysis results MUST be returned as data (e.g., JSON) to be logged or used in a test assertion, with no GUI-based feedback or pop-ups displayed to the user.
- **MANDATORY:** Every E2E test **must validate a complete, multi-step user scenario** derived directly from a user story or acceptance criterion in the `BLUEPRINT.md`, not just a single action or state change.
- **MANDATORY:** E2E tests **must verify the entire data flow** from UI input to the backend/data layer and back to the UI. Mocking of primary internal services or databases within E2E tests is strictly forbidden to prevent shortcutting the integration.
- **MANDATORY:** All visual validations **must combine snapshot testing with state assertions**. The agent must programmatically verify the underlying properties and state of a UI element (e.g., via the accessibility inspector) in addition to capturing its visual appearance to prevent visual-only reward hacking.
- **MANDATORY:** For every feature, the agent **must implement at least one "negative path" E2E test** that validates the system's behaviour with invalid user input, error conditions, or unexpected user flows, ensuring resilience beyond the "happy path."
- **MANDATORY:** Tests **must use dynamic data** generated or fetched from a dedicated test data source. Using hardcoded static values (e.g., writing `"John Doe"` directly into a test for a name field) is prohibited to ensure features work with variable inputs.
- **MANDATORY:** Every E2E test function or file **must include a metadata block explicitly linking it to the requirement ID(s)** in the `BLUEPRINT.md` that it covers. A periodic audit must be run to find blueprint requirements lacking corresponding E2E test coverage.
- **MANDATORY:** For features with subjective or qualitative requirements in the blueprint (e.g., "seamless animation," "intuitive layout"), the E2E test **must include a final step that captures a screen recording or snapshot** and flags it for mandatory review by a designated human or AI judge.
- **MANDATORY:** The agent is strictly **forbidden from adding test-specific conditional logic** (e.g., `if isUITesting { ... }`) into the main application source code. All test accommodations must be made through approved mechanisms like dependency injection or launch arguments.
- **MANDATORY:** Upon the agent's claim of feature completion, an **independent verification process must be automatically triggered**. This process will check out the committed code into a clean environment and re-run all associated E2E tests to confirm the results are reproducible and not dependent on the agent's local state.
- **MANDATORY:** Tests for features with non-functional requirements (NFRs) like performance or responsiveness **must include programmatic assertions for those metrics**. For example, a test for a search feature must not only validate the results but also assert that the search completes under a specified time threshold (e.g., <500ms).
- **MANDATORY:** Every test for an interactive UI element (e.g., Button, Toggle, Slider) **MUST simulate a user interaction** (e.g., `.tap()`, `.swipe()`) and then **assert a verifiable change** in application state, UI, or data model. Tests that only verify an element's existence (`.exists`) are explicitly forbidden and considered incomplete.
- **MANDATORY:** The agent is strictly **prohibited from adding any interactive UI element that cannot be directly traced to a specific, approved requirement** in the `BLUEPRINT.md`. The corresponding commit message for adding the element MUST reference the blueprint requirement ID.
- **MANDATORY:** A **static analysis script MUST be executed before any build test**. This script will automatically fail the process if it detects an interactive UI element whose action handler is empty, contains only placeholder comments, or does not invoke a method on a ViewModel, Controller, or State management object.
- **MANDATORY:** The code within the action handler of any newly created or modified interactive UI element **MUST achieve a minimum of 90% test coverage**. If the action handler is empty or untested, the coverage check will fail, preventing the change from being accepted.
- **MANDATORY:** The implementation of any interactive UI element's action **MUST result in an observable and testable side effect**, such as a state change, a function call on an injected dependency (e.g., a ViewModel), or a navigation event. The unit or integration test for this action must specifically assert that this exact side effect has occurred.
